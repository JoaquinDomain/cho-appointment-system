...
## Environments
One codebase, two Vercel deployments selected by `NEXT_PUBLIC_APP_MODE` (`src/lib/appMode.ts`):
- Patient (public): https://cho-appointment-system-dental-clinic-team.vercel.app — `/` = booking form; `/admin` must 404; no admin link anywhere.
- Admin: https://cho-admin-portal-dental-clinic-team.vercel.app — `/` redirects to `/admin` ("CHO Admin Portal" login + dashboard). Its QR Poster encodes `NEXT_PUBLIC_PATIENT_SITE_URL` (the patient URL), not the admin origin.
...
3. Log in at the admin site `/admin` with that user. RLS grants any `authenticated` user full select/update/delete on `appointments`.
...
- Chrome's window cannot be resized below ~532px wide (WM/Chrome minimum) — use DevTools device emulation (F12 → Ctrl+Shift+M, set 390x844) for phone-viewport tests. If Ctrl+Shift+M doesn't toggle, focus the page first and press again.
- In the emulated mobile viewport, clicking a field then typing can misfire (focus lag / page scroll can send keystrokes to the wrong element, e.g. toggling test checkboxes). Fill ONE field at a time, add a short wait after each click, and zoom-verify each field value before moving on. For the date input, click the MM segment specifically and type digits (e.g. 09152026).
- Both sites should show responsive layouts at 390px: patient form single-column with no horizontal scroll; admin dashboard switches from table (`hidden md:block`) to cards (`md:hidden`) showing name/age/YAKAP badge/date/facility/booked/test chips/UUID.
